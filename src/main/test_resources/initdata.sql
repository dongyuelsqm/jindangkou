...
create table orders(id int primary key AUTO_INCREMENT, discount float, open_id int not null, address_id int not null, deal_date datetime, ship_id varchar(200), state int, express_number varchar(100), method_price FLOAT , actual_price FLOAT) character set = utf8;//修改了数据表的列名，并去除了不需要的列名。//修改了一个拼写错误
