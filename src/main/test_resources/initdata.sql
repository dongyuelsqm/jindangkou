...
create table orders(id int primary key AUTO_INCREMENT, product_id int not null, product_number int not null, discount float, customer_id int not null, address_id int not null, deal_date datetime, ship_id varchar(200), state int, express_number varchar(100), method_price FLOAT , actual_price FLOAT) character set = utf8;
